...
userRouter.post("/register", (req, res) => {
  ...
  User.findOne({ username }, (err, user) => {
    ...
    if (user)
      res.status(400).json({
        message: {
          msgBody: "Username is already taken. Please try again.",
          msgError: true,
        },
      });
    else {
      ...
      newUser.save((err) => {
        if (err)
          res.status(500).json({
            message: {
              msgBody: "Please enter a password.",
              msgError: true,
            },
          });
        else
          res.status(201).json({
            message: {
              msgBody: "Your account has been successfully created.",
              msgError: false,
            },
          });
      });
...
userRouter.delete("/deletemovie/:id", (req, res, next) => {
  Movie.findByIdAndDelete({ _id: req.params.id }, (err, response) => {
    if (err)
      ...
      res.status(200).json({
        message: { msgBody: "Successfully deleted movie", msgError: false },
      });
    }
